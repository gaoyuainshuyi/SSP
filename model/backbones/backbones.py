...
class PromptEncoder(nn.Module):
    def __init__(self, args, channels, bert_out_layers=[3, 6, 9, 12], prompt_levels=[1, 2], **kwargs):
        ...
        stoken_size = [8, 4, 1, 1]
        num_heads = [8, 8, 8, 8]
        self.sp_vis_encoder = stvit_small(args)
        for param in self.sp_vis_encoder.parameters():
    ...
    def forward(self, x, text, l_mask):
        ...
        '''
        # Vis encoding
        vis_outs = [] # pixel features lists
        # super-pixel feats
        sp_feats = self.sp_vis_encoder(x)
        ...
        return vis_outs, sp_feats, l_feats, l_mask
