...
class SRUN(nn.Module):
    ...
    def forward(self, x, text, l_mask, resize_output=True, targets=None, return_probs=False, return_attn=False):
        ...
        if return_attn:
            x, attns = self.pixel_decoder(vis_outs, sp_outs, l_feats, l_mask, return_attn=return_attn) # [B, 1, H, W]
        else:
            x = self.pixel_decoder(vis_outs, sp_outs, l_feats, l_mask) # [B, 1, H, W]
